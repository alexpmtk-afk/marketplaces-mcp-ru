...
SYSTEM_MAP: dict[str, Any] = {
    ...
    "semantic_core": {
        ...
        "archive_executor": "core/semantic_archive.py for WB weekly finance; core/semantic_advertising.py for WB advertising; core/semantic_ozon_sales_returns.py for closed-month Ozon FINAL sales/returns units; core/semantic_ozon_commission.py for Ozon CURRENT commission; core/semantic_ozon_logistics.py for Ozon CURRENT logistics/delivery; core/semantic_ozon_storage.py for Ozon CURRENT storage/placement; core/semantic_ozon_other_adjustments.py for Ozon CURRENT other services and deductions/adjustments",
        ...
        "approved_operational_business_metrics": ["ORDERS", "CURRENT_STOCK", "CURRENT_FBS_STOCK", "CURRENT_SELLING_PRICE", "OZON_BASE_PRICE", "OZON_OLD_PRICE", "OZON_MIN_PRICE", "OZON_FBO_AVAILABLE_STOCK", "OZON_FBO_RESERVED_STOCK", "OZON_FBS_AVAILABLE_STOCK", "OZON_FBS_RESERVED_STOCK", "OZON_ORDERS", "OZON_POSTINGS", "OZON_CANCELLED_POSTINGS"],
        ...
        "current_archive_datasets": ["wb_weekly_finance_main", "ads_campaign_daily", "ads_campaign_roster_snapshots", "ozon_final_realization", "ozon_current_accruals"],
        "current_archive_schema": "WB weekly finance: 92 reviewed physical columns; WB advertising V1: registered campaign daily and campaign-roster schemas; Ozon FINAL realization: closed monthly provider rows with delivery_commission and return_commission components; Ozon CURRENT accruals: open-month rows with posting commission/delivery and typed fee components",
        "resolution_outcomes": [
            "AVAILABLE",
        ...
        "execution_gate": "FULL_COVERAGE from the dataset-specific COMPLETE registry; WB finance uses reports_registry.csv, advertising uses dataset_coverage_registry.csv and roster/fullstats scope proof, Ozon FINAL sales/returns requires one COMPLETE final_coverage_registry month plus exact monthly SHA-256/row-count/stable-key parity, and Ozon CURRENT finance requires COMPLETE current_coverage_registry plus exact SHA-256/row-count/stable-key validation",
        ...
        "rules": [
            ...
            "Ozon cancellations are distinct posting_number rows with status=cancelled; cancellations are not RETURNS and must not be promoted to one order_number status",
            ...
            "Ozon CURRENT commission uses only posting.products[].commission.sale_commission from COMPLETE current accrual coverage; other fee buckets are not substituted",
            "Ozon CURRENT logistics/delivery uses only posting.products[].delivery.total_accrued from COMPLETE current accrual coverage; live verification proved it equals the sum of nested delivery.services[].accrued for the same product lines, so nested services are not added again",
            "Ozon CURRENT storage uses only explicit storage/placement type ids 46, 60, 78, 79 and 102 from COMPLETE current accrual coverage; logistics, commission, advertising and unrelated fees remain separate",
            "Ozon CURRENT other services use only reviewed non-delivery type ids 1, 22, 38, 39, 52, 63, 71, 76 and 84; delivery.services are excluded because they are already inside LOGISTICS_COST",
            "Ozon CURRENT deductions/adjustments use only reviewed type ids 11, 57, 72 and 83; delivery overlap fails closed to prevent double-counting",
    ...
    "routing_policy": {
        ...
        "ozon_cancellations": "OZON_CANCELLED_POSTINGS counts distinct posting_number with provider status=cancelled across selected FBO/FBS feeds. It is not RETURNS and must remain posting-scoped.",
