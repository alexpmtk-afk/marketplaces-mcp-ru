...
if 'BRIDGE_RELEASE_V1 = "1.0.0"' not in s:
...
# 2. Final environment contract. v1 is the normal target; legacy is explicit rollback only.
p = ROOT / '.env.example'
s = p.read_text(encoding='utf-8')
s = s.replace(
...
# 4. Tests must model the stable release, not retired alpha fixtures.
p = ROOT / 'tests/test_archive_google_bridge_v1.py'
s = p.read_text(encoding='utf-8')
