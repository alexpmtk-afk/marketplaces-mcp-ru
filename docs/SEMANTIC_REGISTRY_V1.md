...
Current approved calculations / observations:
...
- `observed_fulfillment_method` → use `rrDate`, return distinct non-empty `deliveryMethod` values observed in the covered archive period, include `officeName` warehouse context, observation count and first/last observed date. The returned data class is `HISTORICAL_OBSERVED_FULFILLMENT`.
...
`deliveryMethod` and `officeName` can answer a different class of question: what fulfillment method and warehouse were actually observed in archived report rows during a covered period.

Rules:

- only non-empty `deliveryMethod` observations are returned;
- `officeName` is context for the historical observation, not a current warehouse configuration;
- `rrDate` defines the archive observation period;
- repeated identical method/warehouse observations are grouped with count plus first/last observed date;
- the result is explicitly classified as `HISTORICAL_OBSERVED_FULFILLMENT`;
- a question containing current-state meaning such as “какая схема сейчас?” never uses this executor and requires another approved current source.
...
7. Archive calculation/observation is allowed only for a capability in `semantic_execution.yaml` and only after `FULL_COVERAGE` is proven.

Examples:

- “По какой схеме отгружался товар?” → approved `observed_fulfillment_method`: return the historical `deliveryMethod` values and warehouse context actually observed in the covered archive period. It does **not** assert the current configuration.
...
- Historical fulfillment execution returns observation evidence only and is marked `HISTORICAL_OBSERVED_FULFILLMENT`.
...
- Canonical archive row inspection confirms separate `deliveryService`, `rebillLogisticCost`, `deduction`, `additionalPayment`, `vw`, `vwNds`, `acquiringFee`, `paymentProcessing`, `acquiringBank`, `deliveryMethod` and `officeName` fields and operation-level values.
